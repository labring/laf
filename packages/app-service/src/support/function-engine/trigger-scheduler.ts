...
export class TriggerScheduler {
  ...
  public emit(event: string, data?: any) {
    ...
    const triggers = this.getEventTriggers().filter(
      (tri) => tri.event === event
    );
    ...
    for (const tri of triggers) {
      logger.debug(
        `trigger scheduler emit ${event} - executing function : ${tri.func_id}`
      );
  ...
  protected timerLoop() {
    ...
    for (const tri of triggers) {
      ...
      if (Date.now() - tri.last_exec_time >= tri.duration * 1000) {
        logger.debug(
          `trigger scheduler timer loop - ${tri.id}- executing function : ${tri.func_id}`
        );
        const param: FunctionContext = {
          params: tri,
          method: "trigger",
